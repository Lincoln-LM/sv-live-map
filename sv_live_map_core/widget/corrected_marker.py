...
class CorrectedMarker(CanvasPositionMarker):
    ...
    def bind_click(self, obj):
        ...
        if self.command is not None:
            self.map_widget.canvas.tag_bind(obj, "<Enter>", self.mouse_enter)
            self.map_widget.canvas.tag_bind(obj, "<Leave>", self.mouse_leave)
            self.map_widget.canvas.tag_bind(obj, "<Double-Button-1>", self.click)

    def in_bounds(self):
        """Check if marker is in bounds"""
        canvas_pos_x, canvas_pos_y = self.get_canvas_pos(self.position)
        return (
            -50 < canvas_pos_x < self.map_widget.width + 50
        ...
        if self.canvas_image is None:
            ...
            if self.scale_with_zoom:
                img = img.resize(
                    (
                        round(
                            self.image.width() * min(1, max(5 / 8, self.last_zoom / 4))
                        ),
                        round(
                            self.image.height() * min(1, max(5 / 8, self.last_zoom / 4))
                        ),
                    ),
                    Image.LANCZOS,
                )
    ...
    def update_canvas_text(self, canvas_pos_x, canvas_pos_y):
        ...
        # move text
            ...
            self.map_widget.canvas.coords(
                self.canvas_text, canvas_pos_x, canvas_pos_y + self.text_y_offset
            )
        ...
        if self.canvas_icon is None:
            ...
            if self.scale_with_zoom:
                img = img.resize(
                    (
                        round(
                            self.icon.width() * min(1, max(5 / 8, self.last_zoom / 4))
                        ),
                        round(
                            self.icon.height() * min(1, max(5 / 8, self.last_zoom / 4))
                        ),
                    ),
                    Image.LANCZOS,
                )
    ...
    def unrender(self):
        ...
        # delete icon canvas shapes
            ...
            self.map_widget.canvas.delete(
                self.polygon, self.big_circle, self.canvas_image
            )
