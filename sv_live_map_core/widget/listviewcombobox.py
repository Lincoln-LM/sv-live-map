        ...
        if event and event.keysym == "BackSpace":
            ...
            if value := self.get():
                for option in self.values:
                    option = str(option)
                    if (
                        option.lower().startswith(value.lower())
                        and option.lower() != value.lower()
                    ):
...
class CTkCombobox(customtkinter.CTkBaseClass):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # border
        self.corner_radius = (
            ThemeManager.theme["shape"]["button_corner_radius"]
            if corner_radius == "default_theme"
            else corner_radius
        )
        self.border_width = (
            ThemeManager.theme["shape"]["entry_border_width"]
        ...
        # font
        self.text_font = (
            (ThemeManager.theme["text"]["font"], ThemeManager.theme["text"]["size"])
        ...
        if not no_color_updates or requires_recoloring or requires_recoloring_2:
            ...
            self.combobox.configure(
                ...
                disabledforeground=ThemeManager.single_color(
                    self.text_color_disabled, self._appearance_mode
                ),
            ...
            if self.state == tkinter.DISABLED:
                ...
                self.canvas.itemconfig(
                    "dropdown_arrow",
                    fill=ThemeManager.single_color(
                        self.text_color, self._appearance_mode
                    ),
                )
    ...
    def on_enter(self, _=None) -> None:
        ...
        if self.hover is True and self.state == tkinter.NORMAL and len(self.values) > 0:
            if sys.platform == "darwin" and Settings.cursor_manipulation_enabled:
                ...
                sys.platform.startswith("win") and Settings.cursor_manipulation_enabled
            ):
            ...
            self.canvas.itemconfig(
                "inner_parts_right",
                outline=ThemeManager.single_color(
                    self.button_hover_color, self._appearance_mode
                ),
                fill=ThemeManager.single_color(
                    self.button_hover_color, self._appearance_mode
                ),
    ...
    def on_leave(self, _=None) -> None:
        ...
        if self.hover and len(self.values) > 0:
            if Settings.cursor_manipulation_enabled and (
            ...
            self.canvas.itemconfig(
                "inner_parts_right",
                outline=ThemeManager.single_color(
                    self.button_color, self._appearance_mode
                ),
                fill=ThemeManager.single_color(
                    self.button_color, self._appearance_mode
                ),
...
class ListViewCombobox(customtkinter.CTkFrame):
    ...
    def __init__(
        self, master, value_enum: Type[IntEnum], *args, width=140, height=28, **kwargs
    ) -> None:
        super().__init__(*args, width=width, height=height, master=master, **kwargs)
