...
class TeraRaidFilterWidget(customtkinter.CTkFrame):
    ...
    def draw_filter_frame(self):
        ...
        self.filter_combobox = customtkinter.CTkComboBox(
            self, values=self.get_filters(), width=500, command=self.select_filter
        )
        ...
        self.nature_label = customtkinter.CTkLabel(self, text="Nature:")
        self.nature_label.grid(row=7, column=0, pady=(10, 0))

        self.nature_filter = CheckedCombobox(self, values=list(Nature))
        self.nature_filter.grid(row=7, column=1, padx=10, pady=(10, 0))

        self.ability_label = customtkinter.CTkLabel(self, text="Ability:")
        self.ability_label.grid(row=8, column=0)

        self.ability_filter = CheckedCombobox(self, values=list(AbilityIndex))
        ...
        self.gender_filter = CheckedCombobox(self, values=list(Gender))
        self.gender_filter.grid(row=9, column=1, padx=10)

        self.difficulty_label = customtkinter.CTkLabel(self, text="Difficulty:")
        self.difficulty_label.grid(row=10, column=0)

        self.difficulty_filter = CheckedCombobox(self, values=list(StarLevel))
        self.difficulty_filter.grid(row=10, column=1, padx=10)

        self.tera_type_label = customtkinter.CTkLabel(self, text="Tera Type:")
        self.tera_type_label.grid(row=11, column=0)

        self.tera_type_filter = CheckedCombobox(self, values=list(TeraType))
        ...
        self.species_label = customtkinter.CTkLabel(self, text="Species:")
        self.species_label.grid(row=1, column=0)

        self.species_filter = ListViewCombobox(self, value_enum=Species)
        self.species_filter.grid(
            row=1, column=1, rowspan=5, padx=10, pady=(10, 0), sticky="n"
        )

        self.reward_label = customtkinter.CTkLabel(self, text="Reward Items:")
        self.reward_label.grid(row=1, column=3)

        self.reward_filter = ListViewCombobox(self, value_enum=Item, width=200)
        self.reward_filter.grid(
            row=1, column=4, rowspan=10, padx=10, pady=(10, 0), sticky="n"
        )

        self.reward_count_filter_label = customtkinter.CTkLabel(
            self, text="Min Reward Count:"
        )
        self.reward_count_filter_label.grid(row=7, column=3, pady=(10, 0))

        self.reward_count_filter = Spinbox(self, width=200, height=28)
        self.reward_count_filter.grid(
            row=7, column=4, padx=10, sticky="n", pady=(10, 0)
        )
    ...
    def new_filter_action(self, name: str):
        ...
        if name is None:
            return
        with open(
            get_path(f"./resources/filter_settings/{name}.json"), "w+", encoding="utf-8"
        ) as new_filter:
            new_filter.write("{}")
    ...
    def save_filter(self):
        ...
        if name := self.filter_combobox.get():
            with open(
                get_path(f"./resources/filter_settings/{name}"), "w+", encoding="utf-8"
            ) as filter_json_file:
    ...
    def load_iv_filter(self, filter_json: dict):
        ...
        self.hp_filter.set_tuple(iv_filters[0])
        self.atk_filter.set_tuple(iv_filters[1])
        self.def_filter.set_tuple(iv_filters[2])
        self.spa_filter.set_tuple(iv_filters[3])
        self.spd_filter.set_tuple(iv_filters[4])
        self.spe_filter.set_tuple(iv_filters[5])

    @staticmethod
    def load_combobox(
        combobox: CheckedCombobox | ListViewCombobox, filter_json: dict, key: str
    ):
        """Load combobox from filter_json"""
        filter_data = filter_json.get(key, [])
        if isinstance(combobox, CheckedCombobox):
            for variable, value in zip(
                combobox.dropdown_menu.variables, combobox.dropdown_menu.values
            ):
    ...
    def get_filter_objects(self) -> list[RaidFilter]:
        ...
        for filename in glob.glob(get_path("./resources/filter_settings/*.json")):
            with open(filename, "r", encoding="utf-8") as file:
                filters.append(RaidFilter.from_json(json.load(file)))
        return filters
