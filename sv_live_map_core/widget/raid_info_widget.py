...
class RaidInfoWidget(customtkinter.CTkFrame):
    ...
    def draw_horizontal_separator(self):
        ...
        self.horizontal_sep.pack(side="top", fill="x", pady=(0, 6))
    ...
    def draw_action_buttons(self, has_alternate_location: bool):
        ...
        if not self.is_popup:
            if has_alternate_location:
                # TODO: tooltip text
                self.swap_location_button = customtkinter.CTkButton(
                    master=self, text="Swap", width=50, command=self.swap_command
                )
            else:
                # padding
                self.swap_location_button = customtkinter.CTkLabel(
                    master=self, text="", width=50
                )
        ...
        self.save_image_button.bind(
            ...
            lambda _: self.save_image_button.bind_all(
                "<Button-2>", lambda _: self.save_image(include_rewards=True)
            ),
        )
        self.save_image_button.bind(
            "<Leave>", lambda _: self.save_image_button.unbind_all("<Button-2>")
        )
    ...
    def create_image(self, include_rewards=False) -> Image.Image:
        ...
        size = (
            ...
            self.winfo_height()
            + (len(self.raid_data.rewards) * 21 + 5 if include_rewards else 0),
        )
        bbox = (0, 0) + size
        ...
        font = ImageFont.truetype(
            get_path("./resources/fonts/Inter/static/Inter-Regular.ttf"), 10
        )
        img_draw.text(
            (
        ...
        if include_rewards:
            ...
            for i, reward in enumerate(self.raid_data.rewards):
                ...
                img.paste(sprite, (5, self.winfo_height() + i * 21 - 8), sprite)
                img_draw.text(
                    (5 + 37, self.winfo_height() + i * 21),
                    f"{reward[1]}x {reward[0]}",
                    font=font,
                    align="center",
                )
                img_draw.text(
                    (5 + 37 + 200, self.winfo_height() + i * 21),
                    f"{reward[2]:img}",
                    font=font,
                    align="center",
                )
                img_draw.text(
                    (5 + 37 + 300, self.winfo_height() + i * 21),
                    f"{reward[3]:img}",
                    font=font,
                    align="center",
                )
    ...
    def draw_info(self, raid_data: TeraRaid):
        ...
        self.info_display = customtkinter.CTkLabel(
            master=self,
            text=raid_data,
            text_font=(customtkinter.ThemeManager.theme["text"]["font"], 8),
            width=200,
        )
    ...
    def draw_main_sprites(self):
        ...
        if self.is_popup:
            self.tera_sprite_display = ImageWidget(
                master=self, image=self.tera_sprite, fg_color=self.fg_color
            )
        else:
        ...
        self.tera_sprite_display.pack(side="left", fill="y", padx=(40, 0))

        self.sprite_display = ImageWidget(
            master=self, image=self.poke_sprite, fg_color=self.fg_color
        )
    ...
    def update_padding(self):
        ...
        self.tera_sprite_display.pack_configure(
            pady=(36 if self.raid_data.hide_sensitive_info else 49, 0)
            if self.is_popup
            else (2 if self.raid_data.hide_sensitive_info else 6, 0)
        )
        self.sprite_display.pack_configure(
            pady=(29 if self.raid_data.hide_sensitive_info else 42, 0)
            if self.is_popup
    ...
    def cache_sprites(self):
        ...
        if len(RaidInfoWidget.TERA_SPRITES) == 0:
            RaidInfoWidget.TERA_SPRITES = [
                ImageTk.PhotoImage(
                    Image.open(get_path(f"./resources/gem/{tera_type.name}.png"))
                )
    ...
    def grab_tera_sprite(self, raid_data):
        ...
        if underlay:
            ...
            basic_sprite = basic_sprite.resize(
                (basic_sprite.height - 4, basic_sprite.width - 4)
            )
