...
class RewardWindow(customtkinter.CTkToplevel):
    ...
    def __init__(
        self, *args, raid_data: TeraRaid = None, fg_color="default_theme", **kwargs
    ):
        super().__init__(*args, fg_color=fg_color, **kwargs)
        assert raid_data is not None
        self.raid_data = raid_data

        self.item_sprite_handler = (
            RewardWindow.ITEM_SPRITE_HANDLER or ItemSpriteHandler(True)
        )
    ...
    def draw_reward(
        ...
        item_sandwich_level_text = customtkinter.CTkLabel(
            master=self, text=str(sandwich_level)
        )
