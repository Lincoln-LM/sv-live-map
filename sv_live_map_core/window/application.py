...
class Application(customtkinter.CTk):
    ...
    def load_settings_and_data(self):
        ...
        with open(
            get_path("./resources/den_locations.json"), "r", encoding="utf-8"
        ) as location_file:
            self.den_locations: dict[str, list[int, int, int]] = json.load(
                location_file
            )
    ...
    def draw_info_frame(self):
        ...
        self.info_frame_horizontal_separator = customtkinter.CTkFrame(
    ...
    def draw_settings_frame(self):
        ...
        self.ip_label = customtkinter.CTkLabel(
            master=self.settings_frame, text="IP Address:"
        )
        ...
        self.usb_check = customtkinter.CTkCheckBox(
            master=self.settings_frame, text="USB"
        )
        ...
        self.use_cached_tables = customtkinter.CTkCheckBox(
            master=self.settings_frame, text="Use Cached Tables"
        )
        ...
        self.scale_sprites_check = customtkinter.CTkCheckBox(
            master=self.settings_frame, text="Scale images with zoom"
        )
        ...
        self.use_filter_check = customtkinter.CTkCheckBox(
            master=self.settings_frame, text="Read with filters"
        )
        ...
        self.raid_progress = customtkinter.CTkProgressBar(
            master=self.settings_frame, width=300
        )
    ...
    def load_from_save(self):
        ...
        filename = filedialog.askopenfilename(
            filetypes=(("Save File", "*"),), initialdir=get_path("./")
        )
        if not filename:
            return
    ...
    def set_window_settings(self):
        ...
        self.geometry(f"{self.WIDTH}x{self.HEIGHT}")
        self.minsize(self.WIDTH, self.HEIGHT)
        self.iconphoto(True, ImageTk.PhotoImage(Image.open(get_path(self.ICON_PATH))))
    ...
    def connect(self) -> bool:
        ...
        try:
            if self.use_cached_tables.get():
                ...
                if 0 in (
                    len(
                        self.reader.raid_enemy_table_arrays[
                            StarLevel.ONE_STAR
                        ].raid_enemy_tables
                    ),
                    len(
    ...
    def read_all_raids(self, render: bool = True) -> RaidBlock:
        ...
        if self.reader:
            ...
            try:
                ...
                if (
                    "position" in self.background_workers
    ...
    def render_raids_work(self, raid_block_data: RaidBlock):
        ...
        def swap_position(raid: TeraRaid):
            ...
            if self.raid_markers[
                raid.id_str
            ].position == self.map_widget.game_coordinates_to_deg(
                *self.den_locations[raid.id_str]
            ):
                # swap the positions of the two markers
                self.raid_markers[raid.id_str].set_position(
                    *self.map_widget.game_coordinates_to_deg(
                        *self.den_locations[other_id_str]
                    )
                )
        ...
        for raid in raid_block_data.raids:
            if raid.is_enabled:
                ...
                if raid.id_str in self.raid_markers:
                    print(
                        f"WARNING duplicate raid id {raid.id_str} is treated as {raid.id_str}_"
                    )
    ...
    def position_work(self):
        ...
        try:
            ...
            game_x, _, game_z = struct.unpack(
                "fff", self.reader.read_main(self.PLAYER_POS_ADDRESS, 12)
            )
        except (TimeoutError, struct.error, binascii.Error, SocketError) as error:
            self.connection_timeout(error)
        ...
        if "marker" not in self.background_workers["position"]:
            ...
            self.background_workers["position"]["marker"] = self.map_widget.set_marker(
                pos_x,
                pos_y,
        ...
        self.background_workers["position"]["worker"] = self.after(
            1000, self.position_work
        )
    ...
    def widget_message_window(
        self, title: str, widget_type: Type[customtkinter.CTkBaseClass], **kwargs
    ...
    def on_closing(self, _=None):
        ...
        for child in self.winfo_children():
            if isinstance(child, customtkinter.CTkToplevel) and hasattr(
                child, "on_closing"
            ):
                child.on_closing()
